            ...
            <motion.div
              ...
              style={
                tier.highlight
                  ? {
                      background:
                        "linear-gradient(160deg, #1740A6 0%, #2563EB 100%)",
                    }
                  : {}
              }
            >
              ...
              <p
                className={cn(
                  "text-xs uppercase tracking-widest font-mono mb-5",
                  tier.highlight ? "text-white/50" : "text-muted",
                )}
              >
                {tier.name}
              </p>
              <div className="mb-1">
                <span
                  className={cn(
                    "font-mono text-[42px] font-bold leading-none",
                ...
                {tier.features.map((f) => (
                  <li key={f} className="flex items-start gap-2.5 text-sm">
                    <Check
                      ...
                      className={cn(
                        "mt-0.5 shrink-0",
                        tier.highlight ? "text-white/80" : "text-pass",
                      )}
                    />
                    <span
                      className={tier.highlight ? "text-white/80" : "text-ink"}
                    >
                      {f}
                    </span>
                  </li>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          >
            ...
            {overages.map((row, i) => (
              <div
                key={row.item}
                className={`flex justify-between px-6 py-3.5 ${i < overages.length - 1 ? "border-b border-border" : ""}`}
              >
