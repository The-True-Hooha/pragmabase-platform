    ...
    <section className="py-28 bg-bg-alt">
      <div className="max-w-[1280px] mx-auto px-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 items-start mb-16">
          ...
          >
            Aegis wraps your agent in a full evaluation harness — sandboxed execution,
            live LLM proxying, and CI-native reporting. Ship with confidence because
            you measured it.
          </motion.p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-0 border border-border rounded-2xl overflow-hidden">
          ...
          >
            ...
            <svg className="absolute inset-0 w-full h-full opacity-10" aria-hidden="true">
              <defs>
                <pattern id="card-grid" x="0" y="0" width="32" height="32" patternUnits="userSpaceOnUse">
            ...
            <div className="relative">
              <p className="text-xs uppercase tracking-widest text-white/50 font-mono mb-5">The Aegis Platform</p>
              <div className="flex flex-wrap gap-2 mb-6">
                {['<2s cold start', '99.9% uptime', '#1 AgentBench'].map((m) => (
                  <span
                    key={m}
            ...
            <button
              className="relative self-start bg-white text-accent px-5 py-2.5 rounded-lg text-sm font-semibold hover:bg-white/90 transition-colors"
            >
              Start building →
            </button>
          </motion.div>
          ...
          >
            <p className="text-xs uppercase tracking-widest text-muted font-mono mb-6">CI-Native Evaluation</p>
            <div>
              {features.map((f, i) => (
                <div
                  key={f.label}
                ...
                >
                  ...
                  <span className="text-xs font-mono text-accent font-medium">{f.value}</span>
                </div>
              ))}
            </div>
          </motion.div>
          ...
          >
            <p className="text-xs uppercase tracking-widest text-muted font-mono">At a glance</p>
            {[
              ...
              <div key={s.lbl}>
                <div className="font-mono font-bold text-3xl text-ink leading-none mb-0.5">{s.val}</div>
                <div className="text-xs uppercase tracking-widest text-muted font-mono">{s.lbl}</div>
              </div>
            ))}
          </motion.div>
        </div>
